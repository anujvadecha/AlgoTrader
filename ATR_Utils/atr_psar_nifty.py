...
def backtest():
    ...
    for index, row in hist_df.iterrows():
        temp = {"instrument_token": spot_data["instrument_token"], "exchange_timestamp": None, "last_price": None,
                "volume_traded": None}
        time = row["date"]
        temp["exchange_timestamp"] = time
        temp["last_price"] = row["open"]
        temp["volume_traded"] = row["cum_vol"] - 300
        if index == 0:
            temp["volume"] = 0
        temp = [temp]
        ...
        ticks.append(temp.copy())
        del temp
    ticks[-1][0]["exchange_timestamp"] = ticks[-1][0]["exchange_timestamp"] + datetime.timedelta(minutes=2)
